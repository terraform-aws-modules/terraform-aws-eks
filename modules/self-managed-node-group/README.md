...
| <a name="input_az_filter"></a> [az\_filter](#input\_az\_filter) | A list of one or more availability zones for the group. Used for EC2-Classic and default subnets when not specified with `subnet_ids` argument. Conflicts with `subnet_ids` | `list(string)` | `null` | no |
