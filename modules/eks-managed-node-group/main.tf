...
locals {
  ...
  placement = var.create && (var.enable_efa_support || var.enable_managed_placement_group) ? { group_name = aws_placement_group.this[0].name } : var.placement
}
...
resource "aws_placement_group" "this" {
  count = var.create && (var.enable_efa_support || var.enable_managed_placement_group) ? 1 : 0

  name     = "${var.cluster_name}-${var.name}"
  strategy = var.managed_placement_group_strategy
