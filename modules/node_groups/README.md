...
| asg\_tags | Tags to be added to the autoscaling group that is generated automatically by AWS as part of the managed node group creation. Each tag must be defined using the key `key` for the tag and `value` for the value. `propagate_at_launch` is optional and defaults to false. See examples.  | list(map(string)) |
