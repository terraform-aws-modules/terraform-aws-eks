...
resource "aws_eks_cluster" "this" {
  ...
  vpc_config {
    security_group_ids      = distinct(concat(var.cluster_additional_security_group_ids, local.cluster_security_group_id_list))
...
locals {
  ...
  cluster_security_group_id_list = local.create_cluster_sg ? [aws_security_group.cluster[0].id] : (var.cluster_security_group_id == "" ? [] : [var.cluster_security_group_id])
