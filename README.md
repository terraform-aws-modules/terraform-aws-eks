...
| <a name="input_cluster_iam_role_name"></a> [cluster\_iam\_role\_name](#input\_cluster\_iam\_role\_name) | IAM role name for the cluster. If set then use fixed IAM role name to create or set for cluster (depending on manage\_cluster\_iam\_resources settings), if not set then create IAM role with cluster\_name as name\_prefix | `string` | `""` | no |
