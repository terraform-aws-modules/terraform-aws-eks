...
| cluster\_log\_kms\_key\_id | If the KMS key arn is provided, the log group will be encrypted with the corresponding KMS key. Please verify that logs has the permission to use the key (https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/encrypt-log-data-kms.html). | string | `""` | no |
