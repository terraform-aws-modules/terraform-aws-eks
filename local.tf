locals {
  ...
  workers_group_defaults_defaults = {
    ...
    ebs_block_device_device_name           = "/dev/xvdb"                     # ebs block device name for worker instances.
    ebs_block_device_snapshot_id           = ""                              # ebs block device snapshot id worker instances.
    ebs_block_device_volume_type           = "gp2"                           # ebs block device volume type for worker instances.
    ebs_block_device_volume_size           = "100"                           # ebs block device volume size for worker instances.
    ebs_block_device_iops                  = "0"                             # ebs block device iops worker instances.
    ebs_block_device_delete_on_termination = true                            # ebs block device delete on termination for worker instances.
    ebs_block_device_encrypted             = false                           # ebs block device encrypted for worker instances.
