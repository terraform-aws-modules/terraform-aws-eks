resource "aws_autoscaling_group" "workers" {
  ...
  launch_configuration = "${element(
                               split(",", (var.worker_group_ebs_block_device_count < 1
                                  ? join(",", aws_launch_configuration.workers_with_zero_ebs_block_devices.*.id)
                                  : (var.worker_group_ebs_block_device_count < 2
                                     ? join(",", aws_launch_configuration.workers_with_one_ebs_block_devices.*.id)
                                     : join(",", aws_launch_configuration.workers_with_two_ebs_block_devices.*.id)))
                               ),
                               count.index
                          )}"
...
resource "aws_launch_configuration" "workers_with_zero_ebs_block_devices" {
  name_prefix                 = "${aws_eks_cluster.this.name}-${lookup(var.worker_groups[count.index], "name", count.index)}"
  ...
  count                       = "${var.worker_group_ebs_block_device_count == 0 ? var.worker_group_count : 0}"
...
resource "aws_launch_configuration" "workers_with_one_ebs_block_devices" {
  name_prefix                 = "${aws_eks_cluster.this.name}-${lookup(var.worker_groups[count.index], "name", count.index)}"
  associate_public_ip_address = "${lookup(var.worker_groups[count.index], "public_ip", local.workers_group_defaults["public_ip"])}"
  security_groups             = ["${local.worker_security_group_id}", "${var.worker_additional_security_group_ids}", "${compact(split(",",lookup(var.worker_groups[count.index],"additional_security_group_ids", local.workers_group_defaults["additional_security_group_ids"])))}"]
  iam_instance_profile        = "${element(aws_iam_instance_profile.workers.*.id, count.index)}"
  ...
  spot_price                  = "${lookup(var.worker_groups[count.index], "spot_price", local.workers_group_defaults["spot_price"])}"
  placement_tenancy           = "${lookup(var.worker_groups[count.index], "placement_tenancy", local.workers_group_defaults["placement_tenancy"])}"
  count                       = "${var.worker_group_ebs_block_device_count == 1 ? var.worker_group_count : 0}"
  ...
  ebs_block_device {
    device_name           = "${lookup(var.worker_groups[count.index], "ebs_block_device_0_device_name", local.workers_group_defaults["ebs_block_device_device_name"])}"
    volume_type           = "${lookup(var.worker_groups[count.index], "ebs_block_device_0_volume_type", local.workers_group_defaults["ebs_block_device_volume_type"])}"
    volume_size           = "${lookup(var.worker_groups[count.index], "ebs_block_device_0_volume_size", local.workers_group_defaults["ebs_block_device_volume_size"])}"
    iops                  = "${lookup(var.worker_groups[count.index], "ebs_block_device_0_iops", local.workers_group_defaults["ebs_block_device_iops"])}"
    snapshot_id           = "${lookup(var.worker_groups[count.index], "ebs_block_device_0_snapshot_id", local.workers_group_defaults["ebs_block_device_snapshot_id"])}"
    delete_on_termination = "${lookup(var.worker_groups[count.index], "ebs_block_device_0_delete_on_termination", local.workers_group_defaults["ebs_block_device_delete_on_termination"])}"
    encrypted             = "${lookup(var.worker_groups[count.index], "ebs_block_device_0_encrypted", local.workers_group_defaults["ebs_block_device_encrypted"])}"
  }
}

resource "aws_launch_configuration" "workers_with_two_ebs_block_devices" {
  name_prefix                 = "${aws_eks_cluster.this.name}-${lookup(var.worker_groups[count.index], "name", count.index)}"
  associate_public_ip_address = "${lookup(var.worker_groups[count.index], "public_ip", local.workers_group_defaults["public_ip"])}"
  security_groups             = ["${local.worker_security_group_id}", "${var.worker_additional_security_group_ids}", "${compact(split(",",lookup(var.worker_groups[count.index],"additional_security_group_ids", local.workers_group_defaults["additional_security_group_ids"])))}"]
  iam_instance_profile        = "${element(aws_iam_instance_profile.workers.*.id, count.index)}"
  ...
  spot_price                  = "${lookup(var.worker_groups[count.index], "spot_price", local.workers_group_defaults["spot_price"])}"
  placement_tenancy           = "${lookup(var.worker_groups[count.index], "placement_tenancy", local.workers_group_defaults["placement_tenancy"])}"
  count                       = "${var.worker_group_ebs_block_device_count == 2 ? var.worker_group_count : 0}"
  ...
  ebs_block_device {
    device_name           = "${lookup(var.worker_groups[count.index], "ebs_block_device_1_device_name", local.workers_group_defaults["ebs_block_device_device_name"])}"
    volume_type           = "${lookup(var.worker_groups[count.index], "ebs_block_device_1_volume_type", local.workers_group_defaults["ebs_block_device_volume_type"])}"
    volume_size           = "${lookup(var.worker_groups[count.index], "ebs_block_device_1_volume_size", local.workers_group_defaults["ebs_block_device_volume_size"])}"
    iops                  = "${lookup(var.worker_groups[count.index], "ebs_block_device_1_iops", local.workers_group_defaults["ebs_block_device_iops"])}"
    snapshot_id           = "${lookup(var.worker_groups[count.index], "ebs_block_device_1_snapshot_id", local.workers_group_defaults["ebs_block_device_snapshot_id"])}"
    delete_on_termination = "${lookup(var.worker_groups[count.index], "ebs_block_device_1_delete_on_termination", local.workers_group_defaults["ebs_block_device_delete_on_termination"])}"
    encrypted             = "${lookup(var.worker_groups[count.index], "ebs_block_device_1_encrypted", local.workers_group_defaults["ebs_block_device_encrypted"])}"
  }
}
