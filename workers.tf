...
resource "aws_autoscaling_group" "workers" {
  count = (var.create_eks || var.create_eks_worker_nodes) ? local.worker_group_count : 0
  name_prefix = join(
    "-",
    compact(
      [
...
resource "aws_security_group" "workers" {
  count       = var.worker_create_security_group && (var.create_eks || var.create_eks_worker_nodes) ? 1 : 0
...
resource "aws_security_group_rule" "workers_ingress_cluster_kubelet" {
  count                    = var.worker_create_security_group && (var.create_eks || var.create_eks_worker_nodes) ? var.worker_sg_ingress_from_port > 10250 ? 1 : 0 : 0
...
resource "aws_iam_role" "workers" {
  count                 = var.manage_worker_iam_resources && (var.create_eks || var.create_eks_worker_nodes) ? 1 : 0
...
resource "aws_iam_instance_profile" "workers" {
  count       = var.manage_worker_iam_resources && (var.create_eks || var.create_eks_worker_nodes) ? local.worker_group_count : 0
  name_prefix = aws_eks_cluster.this[0].name
  role = lookup(
    var.worker_groups[count.index],
    "iam_role_id",
...
resource "aws_iam_role_policy_attachment" "workers_AmazonEKS_CNI_Policy" {
  count      = var.manage_worker_iam_resources && var.attach_worker_cni_policy && (var.create_eks || var.create_eks_worker_nodes) ? 1 : 0
...
resource "aws_iam_role_policy_attachment" "workers_additional_policies" {
  count      = var.manage_worker_iam_resources && (var.create_eks || var.create_eks_worker_nodes) ? length(var.workers_additional_policies) : 0
  role       = aws_iam_role.workers[0].name
