    ...
    content {
      ...
      launch_template {
        launch_template_specification {
          launch_template_id = aws_launch_template.workers_launch_template["${count.index}-default"].id
          version = lookup(
            var.worker_groups_launch_template[count.index],
            ...
            ? aws_launch_template.workers_launch_template["${count.index}-default"].latest_version
            : aws_launch_template.workers_launch_template["${count.index}-default"].default_version
          )
        }
        ...
        dynamic "override" {
          ...
          content {
            ...
            dynamic "launch_template_specification" {
              for_each = lookup(
                lookup(var.worker_groups_launch_template[count.index], "override_instance_ami", {}),
                override.value, null
              ) != null ? [var.worker_groups_launch_template[count.index]["override_instance_ami"][override.value]] : []

              content {
                launch_template_id = aws_launch_template.workers_launch_template["${count.index}-${override.value}"].id
                version = lookup(
                  var.worker_groups_launch_template[count.index],
                  "launch_template_version",
                  lookup(
                  ...
                  ? aws_launch_template.workers_launch_template["${count.index}-${override.value}"].latest_version
                  : aws_launch_template.workers_launch_template["${count.index}-${override.value}"].default_version
                )
              }
            }
...
resource "aws_launch_template" "workers_launch_template" {
  for_each = var.create_eks ? (local.launch_templates_with_instance_ami) : {}
  name_prefix = "${coalescelist(aws_eks_cluster.this[*].name, [""])[0]}-${lookup(
    var.worker_groups_launch_template[each.value.launch_template],
    "name",
  ...
  image_id = each.value.ami == null ? lookup(
    var.worker_groups_launch_template[each.value.launch_template],
    "ami_id",
    lookup(var.worker_groups_launch_template[each.value.launch_template], "platform", local.workers_group_defaults["platform"]) == "windows" ? local.default_ami_id_windows : local.default_ami_id_linux,
  ) : each.value.ami
  ...
  dynamic "elastic_inference_accelerator" {
    ...
    ) != null ? [lookup(var.worker_groups_launch_template[each.value.launch_template], "elastic_inference_accelerator", local.workers_group_defaults["elastic_inference_accelerator"])] : []
    content {
  ...
  user_data = base64encode(
    local.launch_template_userdata_rendered[each.value.launch_template],
  )
  ...
  dynamic "credit_specification" {
    ...
    ) != null ? [lookup(var.worker_groups_launch_template[each.value.launch_template], "cpu_credits", local.workers_group_defaults["cpu_credits"])] : []
    content {
  ...
  dynamic "placement" {
    for_each = lookup(var.worker_groups_launch_template[each.value.launch_template], "launch_template_placement_group", local.workers_group_defaults["launch_template_placement_group"]) != null ? [lookup(var.worker_groups_launch_template[each.value.launch_template], "launch_template_placement_group", local.workers_group_defaults["launch_template_placement_group"])] : []
  ...
  dynamic "instance_market_options" {
    for_each = lookup(var.worker_groups_launch_template[each.value.launch_template], "market_type", null) == null ? [] : tolist([lookup(var.worker_groups_launch_template[each.value.launch_template], "market_type", null)])
    content {
  ...
  block_device_mappings {
    device_name = lookup(
      ...
      lookup(var.worker_groups_launch_template[each.value.launch_template], "platform", local.workers_group_defaults["platform"]) == "windows" ? local.workers_group_defaults["root_block_device_name_windows"] : local.workers_group_defaults["root_block_device_name"],
    )
  ...
  dynamic "block_device_mappings" {
    for_each = lookup(var.worker_groups_launch_template[each.value.launch_template], "additional_ebs_volumes", local.workers_group_defaults["additional_ebs_volumes"])
    content {
  ...
  dynamic "block_device_mappings" {
    for_each = lookup(var.worker_groups_launch_template[each.value.launch_template], "additional_instance_store_volumes", local.workers_group_defaults["additional_instance_store_volumes"])
    content {
      device_name = block_device_mappings.value.block_device_name
      virtual_name = lookup(
        block_device_mappings.value,
  ...
  tag_specifications {
    ...
    tags = merge(
      ...
      { for tag_key, tag_value in var.tags :
        tag_key => tag_value
        if tag_key != "Name" && !contains([for tag in lookup(var.worker_groups_launch_template[each.value.launch_template], "tags", local.workers_group_defaults["tags"]) : tag["key"]], tag_key)
      }
    )
  }
